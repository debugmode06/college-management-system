...
import marksheetRoutes from "./routes/marksheet.js"; // OCR & upload
...
app.use("/api", marksheetRoutes); // OCR & marksheet routes

// 4️⃣ JWT Secret
const JWT_SECRET = "SECRET_KEY"; // For production, use dotenv

// 5️⃣ Create uploads directory if needed
...
// 8️⃣ JWT authentication middleware
...
app.get("/api/arrears", authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    [student_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: "Failed to fetch arrears" });
...
// Marksheet upload
app.post("/api/upload-marksheet", authenticateToken, upload.single("marksheet"), (req, res) => {
  try {
    ...
    db.query(
      ...
      (err, result) => {
        if (err) return res.status(500).json({ error: "Failed to save marksheet record" });
        res.json({
          success: true,
...
app.get("/api/marksheets", authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    [student_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: "Failed to fetch marksheets" });
...
app.get("/api/arrear-questions/:subject_code", authenticateToken, (req, res) => {
